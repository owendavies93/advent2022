...
sub collides {
    ...
    return 0 if $x == $start->[0] && $y == $start->[1];
...
sub run_part {
    my ($start, $end, $initial_time) = @_;

    my ($sx, $sy) = @$start;
    my ($ex, $ey) = @$end;

    my $heur = abs($ex - $sx) + abs($ey - $sy) + $initial_time;
    my $q = List::PriorityQueue->new();
    my $state = [$heur, $sx, $sy, $initial_time];
    $q->insert($state, $heur);
    my $seen = {
        state_key($state) => 1,
    };
...
my $steps = run_part($start, $end, 0);
my $steps_back = run_part($end, $start, $steps);
my $steps_again = run_part($start, $end, $steps_back);
